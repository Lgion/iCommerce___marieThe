...
export default function CameraCapture({ onCapture, onClose, facingMode = 'user' }) {
  ...
  return (
    <div className="cameraCapture">
      <div className="cameraCapture__overlay" onClick={onClose}></div>
      
      <div className="cameraCapture__modal">
        {/* Header */}
        <div className="cameraCapture__header">
          <h3 className="cameraCapture__title"> Prendre une photo</h3>
          <button 
            className="cameraCapture__close-btn"
            onClick={onClose}
            type="button"
          >
        ...
        {/* Contenu principal */}
        <div className="cameraCapture__content">
          {/* Zone de chargement */}
          {isLoading && (
            <div className="cameraCapture__loading">
              <div className="cameraCapture__spinner"></div>
          ...
          {/* Zone d'erreur */}
          {error && (
            <div className="cameraCapture__error">
              <div className="cameraCapture__error-icon"></div>
              <div className="cameraCapture__error-content">
                <h4>Erreur d'accès à la caméra</h4>
                <p>{error}</p>
                <button 
                  className="cameraCapture__retry-btn"
                  onClick={startCamera}
                  type="button"
                >
          ...
          <div className="cameraCapture__preview" style={{ 
            display: (!error && !capturedImage) ? 'block' : 'none' 
          }}>
            <video
              ref={videoRef}
              className="cameraCapture__video"
              autoPlay
              muted
              playsInline
              style={{
            ...
            {/* Overlay de visée */}
            <div className="cameraCapture__viewfinder">
              <div className="cameraCapture__viewfinder-frame"></div>
            </div>
          </div>

          {/* Zone de photo capturée */}
          {capturedImage && (
            <div className="cameraCapture__captured">
              <img 
                src={capturedImage} 
                alt="Photo capturée" 
                className="cameraCapture__captured-image"
              />
            </div>
          )}
          ...
          <canvas
            ref={canvasRef}
            className="cameraCapture__canvas"
            style={{ display: 'none' }}
          />
        </div>

        {/* Controls */}
        <div className="cameraCapture__controls">
          {!capturedImage ? (
            ...
            <>
              {hasPermission && !error && !isLoading && (
                <>
                  <button
                    className="cameraCapture__switch-btn"
                  ...
                  <button
                    className="cameraCapture__capture-btn"
                    onClick={capturePhoto}
                    type="button"
                  >
                    <div className="cameraCapture__capture-btn-inner"></div>
                  </button>
                  
                  <button
                    className="cameraCapture__cancel-btn"
                    onClick={onClose}
                    type="button"
                  >
                    Annuler
            ...
            <>
              <button
                className="cameraCapture__retake-btn"
                onClick={retakePhoto}
                type="button"
              >
              ...
              <button
                className="cameraCapture__confirm-btn"
